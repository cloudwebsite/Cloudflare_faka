          ...
          <DropdownMenu>
            <DropdownMenuTrigger as-child>
              <SidebarMenuButton size="lg" class="h-14 cursor-pointer data-[state=open]:bg-sidebar-accent data-[state=open]:text-sidebar-accent-foreground">
                <img :src="logoUrl" alt="Logo" class="aspect-square size-10 rounded-lg object-contain" />
                <div class="grid flex-1 text-left leading-tight">
                  <span class="truncate text-sm font-semibold">自动发卡</span>
            ...
            <DropdownMenuContent side="right" align="start" :side-offset="8" class="w-64">
              <DropdownMenuLabel>关于 Cloudflare 发卡</DropdownMenuLabel>
              ...
              <DropdownMenuGroup>
                ...
                <DropdownMenuItem as-child>
                  <a href="https://github.com/cloudwebsite/Cloudflare_faka" target="_blank" rel="noopener noreferrer">
        ...
        <SidebarGroupContent>
          <SidebarMenu>
            ...
            <SidebarMenuItem>
              <Collapsible default-open class="group/collapsible">
                <CollapsibleTrigger as-child>
                  <SidebarMenuButton :is-active="isGroupActive(adminNavigation.content)">
                    <NewspaperIcon />
                    <span>{{ adminNavigation.content.title }}</span>
                    <ChevronRightIcon class="ml-auto transition-transform group-data-[state=open]/collapsible:rotate-90" />
                  </SidebarMenuButton>
                </CollapsibleTrigger>
                <CollapsibleContent>
                  <SidebarMenuSub>
                    <SidebarMenuSubItem v-for="item in adminNavigation.content.items" :key="item.title">
                      <SidebarMenuSubButton as-child :is-active="isItemActive(item)">
                        <a :href="basePath + item.path"><span>{{ item.title }}</span></a>
                      </SidebarMenuSubButton>
                    </SidebarMenuSubItem>
...
import { BellIcon, ChevronRightIcon, ChevronUpIcon, ChevronsUpDownIcon, ClipboardListIcon, ExternalLinkIcon, InfoIcon, KeyRoundIcon, LayoutDashboardIcon, LogOutIcon, NewspaperIcon, PackageIcon, SettingsIcon, UserRoundCogIcon, UsersIcon } from "@lucide/vue";
...
async function checkUpdate() {
  ...
  try {
    const response = await fetch("https://raw.githubusercontent.com/cloudwebsite/Cloudflare_faka/main/package.json");
