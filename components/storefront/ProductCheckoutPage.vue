<template>
  <main
    :data-storefront-layout="variant"
    :class="mainClass"
  >
    <!-- Shop-style headers -->
    <header
      v-if="isLanyao"
      class="sticky top-0 z-50 border-b border-border bg-card/95 backdrop-blur"
    >
    ...
    <header
      v-else-if="isDuocai"
      class="storefront-header-brand sticky top-0 z-50"
    >
    ...
    <header
      v-else-if="isQuickcard"
      class="storefront-header-soft sticky top-0 z-50 shrink-0 border-b"
    >
    ...
    <!-- Classic / marketplace / magazine / sidebar header -->
    <header
      v-else
      class="fixed inset-x-0 top-0 z-50 border-b bg-background/95 backdrop-blur"
    >
      <div class="mx-auto flex items-center justify-between px-5" :class="isMarketplace ? 'min-h-12 max-w-7xl' : 'min-h-16 max-w-6xl'">
        <StorefrontBrand />
        <PublicNav />
      </div>
    </header>
    ...
    <div
      :class="isShopStyle
        ? 'pb-10'
        : ['flex min-h-0 flex-1 flex-col overflow-y-auto', isMarketplace ? 'mt-12' : 'mt-16']"
    >
      <!-- ========== Shop-style product layouts ========== -->
      <template v-if="isShopStyle">
        <section
          class="mx-auto w-full flex-1 px-4 pb-10 sm:px-5"
          :class="isLanyao ? 'max-w-7xl pt-6' : isFashion ? 'max-w-6xl pt-6' : 'max-w-6xl pt-6'"
        >
          <div :class="isFashion ? 'flex items-start gap-5' : ''">
            <!-- Fashion left sidebar -->
            <aside
              v-if="isFashion"
              class="hidden w-44 shrink-0 lg:block"
            >
              <div class="sticky top-4 rounded-2xl border border-border bg-card p-4 text-sm">
                <a href="/" class="block font-medium text-foreground hover:underline">返回首页</a>
                <a
                  :href="user ? '/account/order' : '/order'"
                  class="mt-2 block text-muted-foreground hover:underline"
                >{{ messages.nav.orderLookup }}</a>
                <p class="mt-4 text-xs text-muted-foreground">{{ data.categoryName || messages.productCheckout.defaultCategory }}</p>
              </div>
            </aside>

            <div
              class="min-w-0 flex-1"
              :class="isFashion ? 'max-w-5xl rounded-2xl border border-border bg-card p-5 shadow-sm sm:p-6' : ''"
            >
              <div
                class="grid items-start gap-8"
                :class="isLanyao
                  ? 'lg:grid-cols-[minmax(0,1.1fr)_24rem]'
                  : isQuickcard
                    ? 'lg:grid-cols-[minmax(0,1fr)_22rem]'
                    : 'lg:grid-cols-[minmax(0,1.1fr)_22rem]'"
              >
                <article class="min-w-0">
                  <div
                    class="relative overflow-hidden bg-muted"
                    :class="isLanyao || isFashion ? 'rounded-2xl' : isDuocai ? 'rounded-xl' : 'rounded-xl'"
                  >
                    <img
                      :src="data.coverImage || defaultProductImage"
                      :alt="data.name"
                      class="w-full object-cover"
                      :class="isLanyao ? 'max-h-[28rem]' : isQuickcard ? 'max-h-80' : 'max-h-96'"
                    />
                    <Badge
                      v-if="isDuocai && deliveryTypeLabel(selectedSku.deliveryType)"
                      class="absolute left-3 top-3"
                    >
                      {{ deliveryTypeLabel(selectedSku.deliveryType) }}
                    </Badge>
                  </div>
                  ...
                  <div class="mt-5">
                    <h1
                      ...
                      :class="isLanyao ? 'text-2xl sm:text-3xl' : isQuickcard ? 'text-xl sm:text-2xl' : 'text-2xl sm:text-3xl'"
                    >
                      {{ data.name }}
                    </h1>
                    ...
                    <div class="mt-3 flex flex-wrap items-center gap-3 text-sm text-muted-foreground">
                      <span>
                        {{ selectedSku.availableStock === null
                          ? messages.storefront.stockAvailable
                          : isOutOfStock
                            ? messages.storefront.outOfStock
                            : t(messages.storefront.stockCount, { count: selectedSku.availableStock }) }}
                      </span>
                      <span
                        v-if="!isDuocai && deliveryTypeLabel(selectedSku.deliveryType)"
                        class="rounded-md bg-muted px-2 py-0.5 text-xs font-medium text-foreground"
                      >
                        {{ deliveryTypeLabel(selectedSku.deliveryType) }}
                      </span>
                    </div>
                ...
                <aside class="h-fit lg:sticky lg:top-4">
                  <Card :class="isQuickcard || isLanyao ? 'border-border shadow-sm' : ''">
                    <CardHeader>
                      <CardDescription>{{ messages.productCheckout.currentPrice }}</CardDescription>
                      <CardTitle class="storefront-price text-3xl">¥{{ selectedSku.price }}</CardTitle>
                    </CardHeader>
                    <form class="grid min-w-0 gap-6" novalidate @submit.prevent="submit">
                      <CardContent class="grid min-w-0 gap-4">
                        ...
                        <p
                          v-if="requiresPayment && !methods.length"
                          class="rounded-md border border-destructive/30 bg-destructive/5 p-3 text-sm text-destructive"
                        >
                          {{ messages.productCheckout.noPaymentMethod }}
                        </p>
                        <label class="grid min-w-0 gap-2 text-sm font-medium">
                          ...
                          <Input v-model="contactValue" type="email" autocomplete="email" required />
                        </label>
                        <label class="grid min-w-0 gap-2 text-sm font-medium">
                          ...
                          <div class="flex items-center gap-2">
                            <Button
                              ...
                              :disabled="quantity <= selectedSku.minBuy"
                              :aria-label="messages.productCheckout.quantity"
                              @click="quantity = Math.max(selectedSku.minBuy, quantity - 1)"
                            >−</Button>
                            <Input
                              v-model.number="quantity"
                              type="number"
                              class="text-center"
                              :min="selectedSku.minBuy"
                              :max="purchaseLimit"
                              required
                            />
                            <Button
                        ...
                        <FieldSet v-if="selectedSku.deliveryType === 'EXPRESS'" class="min-w-0 gap-4">
                          ...
                          <Field v-if="addresses.length" class="min-w-0">
                            <FieldLabel for="checkout-address-shop">{{ messages.productCheckout.selectSavedAddress }}</FieldLabel>
                            <Select :model-value="selectedAddress" class="block w-full min-w-0" @update:model-value="onAddressSelectionChange">
                              <SelectTrigger id="checkout-address-shop" class="w-full! min-w-0 max-w-full **:data-[slot=select-value]:min-w-0 **:data-[slot=select-value]:flex-1 **:data-[slot=select-value]:truncate">
                                <SelectValue :placeholder="messages.productCheckout.selectShippingAddress" />
                              </SelectTrigger>
                              <SelectContent>
                                <SelectItem v-for="item in addresses" :key="String(item.id)" :value="String(item.id)">{{ item.recipientName }} · {{ item.phone }} · {{ addressSummary(item) }}</SelectItem>
                          ...
                          <div v-if="selectedAddress === 'new' || !addresses.length" class="grid gap-4 sm:grid-cols-2">
                            <VeeField v-for="field in addressFieldsWithoutPostalCode" :key="field.name" v-slot="{ componentField, errors }" :name="field.name" :validate-on-input="true">
                              <Field :class="'wide' in field && field.wide ? 'sm:col-span-2' : ''" :data-invalid="errors.length > 0">
                                <FieldLabel :for="`checkout-shop-${field.name}`"><span v-if="field.required" class="text-destructive">*</span> {{ field.label }}</FieldLabel>
                                <Input :id="`checkout-shop-${field.name}`" v-bind="componentField" :autocomplete="field.autocomplete" :aria-invalid="errors.length > 0" />
                            ...
                            <div class="grid gap-4 sm:col-span-2">
                              <VeeField v-slot="{ componentField, errors }" name="postalCode" :validate-on-input="true">
                                <Field :data-invalid="errors.length > 0">
                                  <FieldLabel for="checkout-shop-postalCode">{{ messages.productCheckout.address.postalCode }}</FieldLabel>
                                  <Input id="checkout-shop-postalCode" v-bind="componentField" autocomplete="postal-code" :aria-invalid="errors.length > 0" />
                                  <FieldError v-if="errors.length" :errors="errors" />
                                </Field>
                              </VeeField>
                        ...
                        <div v-if="discountPreview" class="grid gap-1 rounded-md border bg-muted/30 p-3 text-sm">
                      ...
                      <CardFooter class="flex-col items-stretch gap-3">
                        <Button type="submit" :disabled="loading || isOutOfStock || (requiresPayment && !methods.length)">{{ loading ? messages.productCheckout.processingOrder : isOutOfStock ? messages.storefront.outOfStock : requiresPayment ? messages.productCheckout.submitAndPay : messages.productCheckout.createFreeOrder }}</Button>
              ...
              <!-- Full-width details below buy box (matches demo PC product pages) -->
              <section
                v-if="data.description"
                class="mt-8"
                :class="isFashion ? 'border-t border-border pt-6' : 'rounded-xl border border-border bg-card p-5 sm:p-6'"
              >
                ...
                <div
                  ref="descriptionRef"
                  class="product-rich-content mt-4 text-sm leading-7 text-foreground [&_a]:text-primary [&_a]:underline [&_blockquote]:my-2 [&_blockquote]:border-l-[3px] [&_blockquote]:border-border [&_blockquote]:pl-3 [&_blockquote]:text-muted-foreground [&_h1]:mb-2 [&_h1]:mt-4 [&_h1]:text-2xl [&_h1]:font-bold [&_h2]:mb-2 [&_h2]:mt-4 [&_h2]:text-xl [&_h2]:font-bold [&_h3]:mb-2 [&_h3]:mt-3 [&_h3]:text-[1.1rem] [&_h3]:font-semibold [&_img]:my-4 [&_img]:block [&_img]:max-w-full [&_img]:cursor-zoom-in [&_img]:rounded-lg [&_img]:border [&_img]:transition-opacity [&_img]:hover:opacity-90 [&_li]:my-1 [&_ol]:my-2 [&_ol]:list-decimal [&_ol]:pl-6 [&_p]:my-2 [&_ul]:my-2 [&_ul]:list-disc [&_ul]:pl-6"
                  @click="previewDescriptionImage"
                  v-html="data.description"
                />
                <!-- eslint-enable vue/no-v-html -->
              </section>
              <section
                class="mt-6 rounded-xl border border-border bg-muted/30 p-5 text-sm leading-6 sm:p-6"
              >
                <h2 class="font-semibold">{{ messages.productCheckout.purchaseInstructions }}</h2>
                <div class="mt-3 grid gap-3 text-muted-foreground">
                  <p class="whitespace-pre-wrap">{{ purchaseNote }}</p>
                  <p v-if="deliveryHint" class="whitespace-pre-wrap">{{ deliveryHint }}</p>
                </div>
              </section>
            </div>
          </div>
      ...
      <!-- ========== Classic / marketplace / magazine / sidebar (unchanged structure) ========== -->
      <template v-else>
        <section
          v-if="isMagazine"
        ...
        >
          <div class="mx-auto max-w-6xl px-5 pb-8 pt-10">
            <div class="overflow-hidden rounded-2xl border bg-muted">
              <img :src="data.coverImage || defaultProductImage" :alt="data.name" class="max-h-[28rem] w-full object-cover" />
            </div>
            <div class="mt-6 flex flex-wrap items-end justify-between gap-4">
              <div>
                <Badge variant="secondary">{{ data.categoryName || messages.productCheckout.defaultCategory }}</Badge>
                <h1 class="mt-3 text-3xl font-semibold tracking-normal sm:text-4xl">{{ data.name }}</h1>
                <p v-if="data.subtitle" class="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">{{ data.subtitle }}</p>
              </div>
              <p class="storefront-price text-3xl font-semibold tabular-nums">¥{{ selectedSku.price }}</p>
            </div>
          </div>
        </section>
        <section
          class="mx-auto grid w-full flex-1 gap-8 px-5 pb-10"
          :class="[
            isMarketplace || isWholesale ? 'max-w-7xl pt-6 lg:grid-cols-[minmax(0,1fr)_20rem]' : isMagazine ? 'max-w-6xl pt-6 lg:grid-cols-[minmax(0,1.2fr)_22rem]' : isSidebar ? 'max-w-6xl pt-8 lg:grid-cols-[12rem_minmax(0,1fr)_22rem]' : 'max-w-6xl pt-10 lg:grid-cols-[minmax(0,1fr)_22rem]',
          ]"
        >
          <aside v-if="isSidebar" class="hidden lg:block">
            <div class="sticky top-4 rounded-lg border bg-card p-3 text-sm">
              <a href="/" class="font-medium hover:underline">← {{ messages.storefront.products }}</a>
              <p class="mt-3 text-xs text-muted-foreground">{{ data.categoryName || messages.productCheckout.defaultCategory }}</p>
            </div>
          </aside>
          <article>
            <div v-if="!isMagazine" class="overflow-hidden border bg-muted" :class="isMarketplace ? 'rounded-md' : 'rounded-xl'">
              <img :src="data.coverImage || defaultProductImage" :alt="data.name" class="w-full object-cover" :class="isMarketplace ? 'max-h-72' : 'max-h-96'" />
            </div>
            <div v-if="!isMagazine" class="mt-5">
              <Badge variant="secondary">{{ data.categoryName || messages.productCheckout.defaultCategory }}</Badge>
              <h1 class="mt-3 font-semibold tracking-normal" :class="isMarketplace ? 'text-xl sm:text-2xl' : 'text-2xl sm:text-3xl'">{{ data.name }}</h1>
              <p v-if="data.subtitle" class="mt-2 text-sm leading-6 text-muted-foreground">{{ data.subtitle }}</p>
            </div>
            <section v-if="data.description" class="mt-6 border bg-card p-5 sm:p-6" :class="isMarketplace ? 'rounded-md' : 'rounded-xl'">
              <h2 class="text-base font-semibold">{{ messages.productCheckout.productDetails }}</h2>
              <!-- `description` is sanitized server-side by sanitizeProductDescription(). -->
              <!-- eslint-disable vue/no-v-html -->
            ...
            <section class="mt-6 border bg-muted/30 p-5 text-sm leading-6 sm:p-6" :class="isMarketplace ? 'rounded-md' : 'rounded-xl'">
              <h2 class="font-semibold">{{ messages.productCheckout.purchaseInstructions }}</h2>
          ...
          <aside class="h-fit lg:sticky lg:top-0">
            <Card :class="isMarketplace ? 'shadow-sm' : ''">
              <CardHeader><CardDescription>{{ messages.productCheckout.currentPrice }}</CardDescription><CardTitle class="storefront-price text-3xl">¥{{ selectedSku.price }}</CardTitle></CardHeader><form class="grid min-w-0 gap-6" novalidate @submit.prevent="submit">
                <CardContent class="grid min-w-0 gap-4">
                  <div v-if="data.skus.length > 1" class="grid gap-2 text-sm font-medium">
                    <span>选择规格</span>
                    <div class="grid grid-cols-1 gap-2 sm:grid-cols-2" role="group" aria-label="选择规格">
...
import type { Data } from "@/pages/product/@slug/+data.server";
import type { StorefrontLayoutId } from "@/lib/storefront-templates/types";

const props = withDefaults(defineProps<{ variant?: StorefrontLayoutId }>(), { variant: "classic" });
const { messages, t } = useStorefrontPreferences();
const data = useData<Data>();
const isMarketplace = computed(() => props.variant === "marketplace");
const isMagazine = computed(() => props.variant === "magazine");
const isSidebar = computed(() => props.variant === "sidebar");
const isWholesale = computed(() => props.variant === "wholesale");
const isLanyao = computed(() => props.variant === "lanyao");
const isDuocai = computed(() => props.variant === "duocai");
const isFashion = computed(() => props.variant === "fashion");
const isQuickcard = computed(() => props.variant === "quickcard");
const isShopStyle = computed(() => isLanyao.value || isDuocai.value || isFashion.value || isQuickcard.value);
const mainClass = computed(() => {
  // Prefer normal document scroll so full-width 商品详情 stays reachable on PC.
  if (isShopStyle.value) return "min-h-screen bg-background text-foreground";
  return "flex h-dvh flex-col overflow-hidden bg-muted/30";
});
