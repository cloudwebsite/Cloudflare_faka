...
function resolveCategoryIcon(value: unknown) {
  const image = optionalText(value, 2_048, "CATEGORY_ICON_INVALID");
  if (!image) return null;
  if (image.startsWith("/media/proxy/")) return image;
  try {
    const url = new URL(image);
  ...
  appError("CATEGORY_ICON_INVALID");
}
...
async function internalOnGetCatalogAdminData(input: ProductListQuery = {}) {
  ...
  const [categories, items, totalRows] = await Promise.all([
    db.select({ id: category.id, name: category.name, slug: category.slug, description: category.description, icon: category.icon, sort: category.sort, status: category.status }).from(category).orderBy(asc(category.sort), asc(category.id)),
...
async function internalOnSaveCategory(input: { id?: number; name: string; slug?: string; description?: string; icon?: string | null; sort: number }) {
  const { db } = getAdminDb();
  const now = new Date();
  ...
  const values = {
    ...
    icon: resolveCategoryIcon(input.icon),
