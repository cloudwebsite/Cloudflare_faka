...
import { parseBepusdtConfig, parseEpayConfig, parseHashpayConfig, parseStripeConfig, parsePerpayConfig, parsePaypalConfig } from "@/lib/config-schemas";
...
export function createProviderAdapter(provider: PaymentProviderKind, config: Record<string, unknown>): PaymentAdapter {
  ...
  if (provider === "PAYPAL") return createPaypalAdapter(parsePaypalConfig(json));
