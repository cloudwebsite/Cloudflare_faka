...
export function createPerpayAdapter(config: PerpayConfig) {
  return {
    create: async (input: { orderNo: string; amount: number; subject: string; notifyUrl: string; returnUrl: string }) => {
      const requestBody = { idempotency_key: `faka:${input.orderNo}`, merchant_order_no: input.orderNo, amount_cents: input.amount, product_name: input.subject, ...(input.notifyUrl ? { notify_url: input.notifyUrl } : {}), ...(input.returnUrl ? { return_url: input.returnUrl } : {}) };
