...
export type PaymentProviderKind = "ALIPAY" | "EPAY" | "BEPUSDT" | "STRIPE" | "HASHPAY" | "PERPAY" | "PAYPAL";
...
export const paymentProviderDefinitions: Record<PaymentProviderKind, ProviderDefinition> = {
  ...
  PAYPAL: {
    ...
    fields: [
      ...
      { key: "environment", label: "环境", type: "select", required: true, options: [{ label: "Sandbox", value: "sandbox" }, { label: "Live", value: "live" }] },
      { key: "currency", label: "收款币种", type: "select", required: true, options: [{ label: "USD", value: "USD" }, { label: "EUR", value: "EUR" }, { label: "GBP", value: "GBP" }, { label: "HKD", value: "HKD" }, { label: "JPY", value: "JPY" }] },
      { key: "cnyPerUnit", label: "固定汇率（人民币/单位）", type: "text", required: true, placeholder: "7.25", description: "1 单位收款币种等于多少人民币。例：7.25 表示 1 USD = ¥7.25。修改汇率后，未完成的 PayPal 订单可能无法自动入账。" },
      { key: "webhookId", label: "Webhook ID", type: "text", description: "可空。填写后才校验 PayPal Webhook 签名；留空则仅依赖主动查询入账。" },
      common.notifyUrl,
      common.returnUrl,
    ],
    defaults: { schemaVersion: 1, clientId: "", clientSecret: "", environment: "sandbox", currency: "USD", cnyPerUnit: "7.25", webhookId: "", notifyUrl: "", returnUrl: "" },
    parseConfig: parsePaypalConfig,
    getChannels: () => [],
    createAdapter: (config) => createProviderAdapter("PAYPAL", config),
    callbackResponse: "text",
  },
};
...
const paymentNotifyPaths: Partial<Record<PaymentProviderKind, string>> = {
  ...
  PAYPAL: "/api/payments/paypal/notify",
};
