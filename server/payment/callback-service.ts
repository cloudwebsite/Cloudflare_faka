...
export class PaymentCallbackService {
  ...
  async handle(provider: PaymentProviderKind, input: { payload: Record<string, string>; rawBody?: string; rawBodyBytes?: Uint8Array; headers?: Headers }) {
    ...
    // PayPal webhook auth has no CNY amount; finalize via query with the local order amount + fixed rate.
    if (provider === "PAYPAL" && result.verified && result.orderNo && record && result.status === "PAID" && definition.createAdapter(config).query) {
      const paymentOrderNo = result.paymentOrderNo ?? attempt?.paymentOrderNo ?? undefined;
      result = await definition.createAdapter(config).query!({ orderNo: result.orderNo, paymentOrderNo, amount: record.amount });
    }
