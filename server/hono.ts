...
import { handleTelegramSupportWebhook } from "./telegram-support/handler";
...
function getApp() {
  ...
  for (const [provider, path] of [["ALIPAY", "/api/payments/alipay/notify"], ["EPAY", "/api/payments/epay/notify"], ["BEPUSDT", "/api/payments/bepusdt/notify"], ["STRIPE", "/api/payments/stripe/notify"], ["HASHPAY", "/api/payments/hashpay/notify"], ["PERPAY", "/api/payments/perpay/notify"], ["PAYPAL", "/api/payments/paypal/notify"]] as const) {
  ...
  app.post("/api/telegram/support", async (context) => handleTelegramSupportWebhook(context.env.DB, context.req.raw));
