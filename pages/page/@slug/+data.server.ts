...
export async function data(pageContext: { routeParams: { slug: string } }) {
  return withServerDataErrorHandling("page data: page detail", pageContext, async () => {
    const article = await getPublishedArticleBySlug(env.DB, "PAGE", pageContext.routeParams.slug);
