<template>
  <div class="grid gap-2 rounded-md border p-2">
    ...
    <Textarea v-if="htmlMode" v-model="htmlDraft" :class="compact ? 'min-h-40 resize-y font-mono text-sm' : 'min-h-80 resize-y font-mono text-sm'" aria-label="HTML 源码" @update:model-value="updateHtml" />
    <EditorContent v-else :editor="editor" :class="compact ? 'product-editor-content product-editor-content--compact min-h-40 px-2 py-1' : 'product-editor-content min-h-80 px-2 py-1'" />
...
const editor = useEditor({ content: props.modelValue, extensions: [StarterKit, Link.configure({ openOnClick: false }), Image.configure({ allowBase64: false }), TextStyle, Color.configure({ types: ["textStyle"] }), Highlight.configure({ multicolor: true }), Placeholder.configure({ placeholder: props.placeholder })], onUpdate: ({ editor: instance }) => emit("update:modelValue", instance.getHTML()) });
const formatTools = [
...
.product-editor-content--compact .ProseMirror { min-height: 10rem; }
