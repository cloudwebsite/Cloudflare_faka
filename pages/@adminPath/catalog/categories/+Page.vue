  ...
  <section class="flex w-full flex-col gap-6">
    ...
    <AdminDataTable :columns="columns" :rows="paginatedCategories" row-key="id">
      ...
      <template #cell-icon="{ row }">
        <img v-if="row.icon" :src="row.icon" :alt="row.name" class="size-8 rounded border object-cover" />
        ...
        <form class="grid gap-4" @submit.prevent="saveCategory">
          ...
          <div class="grid gap-2">
            ...
            <div class="flex gap-2">
              <Input id="category-icon" v-model="form.icon" placeholder="/media/proxy/... 或 https://..." />
              <Button type="button" variant="outline" @click="mediaPickerOpen = true">选择图片</Button>
            </div>
            <img v-if="form.icon" :src="form.icon" alt="" class="mt-1 size-12 rounded border object-cover" />
          </div>
    ...
    <MediaPickerDialog :open="mediaPickerOpen" @update:open="(open) => { mediaPickerOpen = open; }" @select="(url) => { form.icon = url; mediaPickerOpen = false; }" />
  </section>
</template>
...
const columns: AdminTableColumn<Category>[] = [
  ...
  { key: "icon", label: "图标", class: "w-16", headerClass: "w-16" },
...
const form = reactive({ id: undefined as number | undefined, name: "", slug: "", description: "", icon: "", sort: 0 });
...
function openEdit(item: Category) {
  ...
  Object.assign(form, { id: item.id, name: item.name, slug: item.slug, description: item.description ?? "", icon: item.icon ?? "", sort: item.sort });
  dialogOpen.value = true;
}
async function saveCategory() { saving.value = true; try { await runTelefunc(() => onSaveCategory({ ...form, icon: form.icon || null })); dialogOpen.value = false; resetForm(); await loadCategories(); } catch { /* runTelefunc 已显示统一错误提示。 */ } finally { saving.value = false; } }
...
function resetForm() {
  slugTouched.value = false;
  Object.assign(form, { id: undefined, name: "", slug: "", description: "", icon: "", sort: 0 });
}
