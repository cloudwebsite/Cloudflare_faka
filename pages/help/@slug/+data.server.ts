...
export async function data(pageContext: { routeParams: { slug: string } }) {
  return withServerDataErrorHandling("page data: help detail", pageContext, async () => {
    const article = await getPublishedArticleBySlug(env.DB, "HELP", pageContext.routeParams.slug);
