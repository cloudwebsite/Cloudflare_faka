-- Initial business data for a fresh FAKA installation.
...
INSERT INTO `paymentProvider` (
  ...
  ('PAYPAL', 'PayPal', false, 70, '{"schemaVersion":1,"clientId":"","clientSecret":"","environment":"sandbox","currency":"USD","cnyPerUnit":"7.25","webhookId":"","notifyUrl":"","returnUrl":""}', unixepoch('now') * 1000, unixepoch('now') * 1000)
...
-- Sample help article for order lookup. Idempotent by (type, slug).
INSERT INTO `site_article` (
  `type`, `title`, `slug`, `summary`, `bodyHtml`, `status`, `pinned`, `sort`, `publishedAt`, `createdAt`, `updatedAt`
)
SELECT
  'HELP',
  '如何查询订单',
  'how-to-query-order',
  '使用订单号和下单邮箱在订单查询页查看订单状态与发货内容。',
  '<p>匿名下单后，请保存好您的<strong>订单号</strong>与<strong>下单邮箱</strong>。</p><ol><li>打开站点「订单查询」页面。</li><li>填写完整订单号（以 ORD 开头）以及下单时使用的邮箱。</li><li>提交后即可查看支付状态；若已发货，将显示发货内容。</li></ol><p>已登录用户可在「我的订单」中查看本人订单，无需再次填写邮箱。</p><p>如仍无法查询，请通过页脚客服联系方式联系站长，并提供订单号。</p>',
  'PUBLISHED',
  1,
  0,
  unixepoch('now') * 1000,
  ...
  unixepoch('now') * 1000
WHERE NOT EXISTS (
  SELECT 1 FROM `site_article` WHERE `type` = 'HELP' AND `slug` = 'how-to-query-order'
);
