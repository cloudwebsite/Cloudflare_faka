...
export type PaypalConfig = {
  ...
  /** 1 unit of currency equals this many CNY, e.g. "7.25". */
  cnyPerUnit: string;
  webhookId: string;
  notifyUrl: string;
  returnUrl: string;
...
export type PaymentProviderConfig = AlipayConfig | EpayConfig | BepusdtConfig | StripeConfig | HashpayConfig | PerpayConfig | PaypalConfig;
...
function requirePaypalRate(value: unknown, field: string) {
  const text = requireString(value, field);
  if (!/^\d+(\.\d{1,6})?$/.test(text)) throw new Error(`Invalid configuration: ${field} must be a positive decimal with up to 6 places`);
  const rate = Number(text);
  if (!Number.isFinite(rate) || rate <= 0) throw new Error(`Invalid configuration: ${field} must be a positive decimal with up to 6 places`);
  return text;
}

export function parsePaypalConfig(json: string): PaypalConfig {
  const value = parseJsonObject(json, "PayPal");
  requireSchemaVersion(value, "schemaVersion");
  const currency = requireString(value.currency, "currency").toUpperCase();
  if (!["USD", "EUR", "GBP", "HKD", "JPY"].includes(currency)) throw new Error("Invalid configuration: currency is not supported");
  const environment = requireString(value.environment, "environment");
  if (environment !== "sandbox" && environment !== "live") throw new Error("Invalid configuration: environment must be sandbox or live");
  return {
    schemaVersion: 1,
    clientId: requireString(value.clientId, "clientId"),
    clientSecret: requireString(value.clientSecret, "clientSecret"),
    environment,
    currency: currency as PaypalCurrency,
    cnyPerUnit: requirePaypalRate(value.cnyPerUnit, "cnyPerUnit"),
    webhookId: typeof value.webhookId === "string" ? value.webhookId.trim() : "",
    notifyUrl: requirePaymentUrl(value.notifyUrl, "notifyUrl", true),
    returnUrl: requirePaymentUrl(value.returnUrl, "returnUrl", true),
  };
}
