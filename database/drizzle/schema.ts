...
export const siteSetting = sqliteTable("siteSetting", {
  ...
  storefrontLayout: text("storefrontLayout").notNull().default("classic"),
  storefrontSkin: text("storefrontSkin").notNull().default("default"),
  createdAt,
  updatedAt,
});
...
export const storefrontBanner = sqliteTable(
  "storefront_banner",
  {
    id: integer("id").primaryKey({ autoIncrement: true }),
    title: text("title"),
  ...
  (table) => [index("storefront_banner_status_sort_idx").on(table.status, table.sort)],
);

export const siteArticle = sqliteTable(
  "site_article",
  {
    id: integer("id").primaryKey({ autoIncrement: true }),
    type: text("type", { enum: ["NEWS", "HELP", "PAGE"] }).notNull(),
  ...
  (table) => [
    uniqueIndex("site_article_type_slug_unique").on(table.type, table.slug),
    index("site_article_type_status_pin_sort_idx").on(table.type, table.status, table.pinned, table.sort, table.publishedAt),
  ],
);
...
export const telegramSupportConfig = sqliteTable("telegramSupportConfig", {
...
export const telegramSupportSession = sqliteTable(
  "telegramSupportSession",
  {
    chatId: text("chatId").primaryKey(),
    state: text("state", { enum: ["idle", "await_order", "await_email", "human"] }).notNull().default("idle"),
    pendingOrderNo: text("pendingOrderNo"),
    updatedAt,
  },
  (table) => [index("telegramSupportSession_updatedAt_idx").on(table.updatedAt)],
);
